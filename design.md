...
| **품질 lead** | 명백한 버그, 테스트 커버리지, 타입 안정성, 데드 코드, 에러 핸들링 | 명백한 버그 또는 새 로직에 테스트 0 인 경우만 | 끝없는 nitpick. *"blocking 으로 표시하려면 사용자 영향 또는 정확성 위반을 명시해야 한다"* — specialist 없이 lead 단독 처리 |
