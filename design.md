# agent-secretary — 멀티-에이전트 시스템 설계

이 문서는 두 종류의 에이전트 워크플로우를 다루는 시스템의 설계를 담는다. 구현 에이전트는 이 문서 + [`design_server.md`](design_server.md) 를 읽고 골격을 만들 수 있어야 한다.

1. **PR 리뷰 (`pr_review`, Phase 1: 섀도우 모드)** — GitHub PR webhook 트리거. 멀티 페르소나 + CTO 메타-판단으로 *자동 머지 / 변경 요청 / 사람 에스컬레이션* 중 하나를 결정. **단, Phase 1 에서는 PR 코멘트만 게시하고 어떠한 GitHub 상태 변경도 하지 않는다** — 보정 데이터 수집 우선.
2. **Slack-트리거 워크플로우 (`code_analyze`, `code_modify`, `linear_issue`)** — Slack 채널에서 `@봇 분석 / 수정 / 이슈 등록` 으로 호출. 사용자가 *명시적으로* 요청하므로 PR 리뷰의 "액션 없음" 원칙은 적용되지 않음. 자세한 워크플로우 정의는 `prompts/workflows/` 참조.
...
> 아래 §2~§13 은 PR 리뷰 시스템에 한정된 내용이다. Slack-트리거 워크플로우는 이 시스템과 같은 4-레이어 파이프라인 (ingress / core / agents / egress) 에서 *별개의 워크플로우 타입*으로 동작하며, 페르소나 / CTO 를 거치지 않는 단일-에이전트 호출이다.
