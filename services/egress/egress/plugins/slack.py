...
Posts the workflow result back to the originating Slack thread:
  ...
  - Posts `summary_markdown` as a thread message.
  - If `detail_markdown` is present, attaches it as `result.md`.

Reaction state on Slack is best-effort — if a reaction call fails (already
removed, message gone, permission), we log and continue. The result message
itself is the user-facing signal of completion.
"""
...
class SlackDeliverer(ChannelDeliverer):
    ...
    async def deliver(self, result: ResultEvent) -> None:
        ...
        # 2. Post the message + (optional) detail file in-thread.
        try:
            if result.detail_markdown:
                await self._web.files_upload_v2(
                    channel=channel_id,
                    thread_ts=thread_ts,
                    content=result.detail_markdown,
                    filename="result.md",
                    filetype="markdown",
                    initial_comment=result.summary_markdown,
                )
                log.info(
                    "slack.deliver.file_uploaded",
                    result_id=result.result_id,
                    channel=channel_id,
                )
            else:
